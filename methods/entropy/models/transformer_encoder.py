...
class VisionTransformer(nn.Module):
    ...
    def split_input(self, tokens, patches_scale_coords, curr_scale, patch_size, im):
        ...
        tokens_to_split, coords_to_split, tokens_to_keep, coords_to_keep = self.divide_tokens_to_split_and_keep(tokens_at_curr_scale, coords_at_curr_scale, curr_scale)
    ...
    def forward(self, im):
        ...
        for l_idx in range(len(self.layers)):
            ...
            if l_idx < self.n_scales - 1: 
                x, patches_scale_coords = self.split_input(x, patches_scale_coords, l_idx, patched_im_size, im)
                PS /= 2
                patched_im_size *= 2
