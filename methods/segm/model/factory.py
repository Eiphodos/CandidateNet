...
def create_vit(model_cfg):
    ...
    default_cfgs = default_cfgs_vit

    if backbone in default_cfgs:
        default_cfg = default_cfgs[backbone]
    elif 'deit_base_distilled_patch16_384' in backbone:
        default_cfg = dict(url='https://dl.fbaipublicfiles.com/deit/deit_base_distilled_patch16_384-d0272ac0.pth',
                           num_classes=1000,
                           input_size=(3, 384, 384),
                           pool_size=None,
                           crop_pct=1.0,
    ...
    default_cfg["input_size"] = input_size

    model = VisionTransformer(**model_cfg)
    if backbone == "vit_base_patch8_384":
        path = os.path.expandvars("$TORCH_HOME/hub/checkpoints/vit_base_patch8_384.pth")
        state_dict = torch.load(path, map_location="cpu")
        filtered_dict = checkpoint_filter_fn_deit(state_dict, model)
        model.load_state_dict(filtered_dict, strict=True)
    elif "deit" in backbone:
        load_pretrained(model, default_cfg, filter_fn=checkpoint_filter_fn_deit)
    else:
...
def create_segmenter(model_cfg):
    ...
    model = Segmenter(encoder, decoder, n_cls=model_cfg["n_cls"], model_cfg=model_cfg)
