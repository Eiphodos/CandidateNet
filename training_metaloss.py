...
def main(args):
    ...
    for e in range(args.epochs):
        ...
        print("Train Epoch: {}, Dice score: {:.4f}, IoU score: {:.4f}, dice_ce_loss: {:.4f}, meta_loss: {:.4f}, epoch_time: {:.4f}".format(e, dice_scores.item(), iou_scores.item(), dice_ce_loss.item(), meta_loss.item(), epoch_time))
        if wandb.run is not None:
            wandb.log({"train/epoch": e, 'train/dice': dice_scores.item(), 'train/iou': iou_scores.item(), 'train/loss': loss.item(), 'train/dice_ce_loss': dice_ce_loss.item(), 'train/meta_loss': meta_loss.item()}, step=e)
        scheduler.step()
