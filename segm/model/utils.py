...
def checkpoint_filter_fn_deit(state_dict, model):
...
def resize(im, smaller_size):
    ...
    if min(h, w) < smaller_size:
        im_res = F.interpolate(im, (int(h_res), int(w_res)), mode="bilinear", align_corners=False)
    else:
        im_res = im
    return im_res
    ...
    for im, im_metas in zip(ims, ims_metas):
        ...
        with torch.no_grad():
            for i in range(0, B, WB):
                seg_maps[i : i + WB], _, _ = model.forward(crops[i : i + WB])
